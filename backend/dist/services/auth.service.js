...
class AuthService {
    async signUp(user) {
        ...
        await emailService.sendMail(newUser.email, emailConstants[EmailEnum.ACTIVATE], {
            name: newUser.name,
    ...
    async signIn(dto) {
        ...
        const isValidPass = await passwordService.comparePass(dto.password, user.password);
        if (!user.isActive) {
    ...
    async recoveryPasswordRequest(user) {
        ...
        await emailService.sendMail(user.email, emailConstants[EmailEnum.RECOVERY], {
            url,
        });
    }
