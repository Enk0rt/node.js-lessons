...
class AuthMiddleware {
    async checkAccessToken(req, res, next) {
        try {
            ...
            const isTokenExists = await tokenService.isExists(accessToken, TokenTypeEnum.ACCESS);
            if (!isTokenExists) {
    ...
    async checkRefreshToken(req, res, next) {
        try {
            ...
            if (!refreshToken) {
                throw new ApiError("refreshToken is not provided", StatusCodesEnum.FORBIDDEN);
            }
            const tokenPayload = tokenService.verifyToken(refreshToken, TokenTypeEnum.REFRESH);
            const isTokenExists = await tokenService.isExists(refreshToken, TokenTypeEnum.REFRESH);
            if (!isTokenExists) {
